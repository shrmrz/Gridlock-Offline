...
function TrafficLightControlEditor(trafficObjects, xRelEditor, yRelEditor) {
  ...
  for (var i = 0; i < trafficObjects.trafficObj.length; i++) {
    ...
    if (trafficObj.type === "trafficLight") {
      ...
      void (
        debug &&
        console.log(
          "TrafficLightControlEditor Cstr: iTL=",
          iTL,
          " this.doubleSliders[iTL]=",
          this.doubleSliders[iTL]
        )
      );
      this.doubleSliders[iTL].toRed.relValue = Math.max(
        0,
        Math.min(1, this.doubleSliders[iTL].toRed.relValue)
      ); //!!!
      this.doubleSliders[iTL].toGreen.relValue = Math.max(
        0,
        Math.min(1, this.doubleSliders[iTL].toGreen.relValue)
      ); //!!!
...
TrafficLightControlEditor.prototype.getSlider = function (id) {
  ...
  console.log(
    "TrafficLightControlEditor.getSlider: no slider found",
    " with id",
    id
  );
  return null;
...
TrafficLightControlEditor.prototype.resize = function (canvas) {
  ...
  this.dySlider = 0.1 * this.wrel * sizeCanvas; // dist between sliders 0.1 panel width
};
  ...
  for (var i = 0; i < trafficObjects.trafficObj.length; i++) {
    ...
    if (trafficObj.type === "trafficLight") {
      ...
      if (trafficObj.isActive) {
        ...
        slider.toGreen.xyPix = this.get_xyPix(
          iDisplay,
          slider.toGreen.relValue
        );
        iDisplay++;
      } else {
        // reset slider if dragged from road
...
TrafficLightControlEditor.prototype.get_xyPix = function (xrel, yrel) {
  var sizeCanvas = Math.min(canvas.width, canvas.height);
  var xPix =
    canvas.width * this.xRelEditor + sizeCanvas * this.wrel * (xrel - 0.5);
  var yPix =
    canvas.height * (1 - this.yRelEditor) -
    sizeCanvas * this.hrel * (yrel - 0.5);
  return [xPix, yPix];
};
...
TrafficLightControlEditor.prototype.get_xyPixSlider = function (
  iDisplay,
  valRel
) {
  var sizeCanvas = Math.min(canvas.width, canvas.height);
  var xrelBeginSlider = 0.2; // all relative to editor panel
...
TrafficLightControlEditor.prototype.mouseIsInside = function (
  xPixUser,
  yPixUser
) {
...
TrafficLightControlEditor.prototype.pick = function (xyPixUser, xyPixObj) {
  var sizeCanvas = Math.min(canvas.width, canvas.height);
  var dist2 =
    Math.pow(xyPixUser[0] - xyPixObj[0], 2) +
...
TrafficLightControlEditor.prototype.selectCycleTime = function (xyPixUser) {
  var success = false;
...
TrafficLightControlEditor.prototype.showEditPanel = function () {
  ...
  var hSlider =
    0.6 * (this.get_xyPixSlider(1, 0)[1] - this.get_xyPixSlider(0, 0)[1]);
  ...
  var xyPix = this.get_xyPix(0.03, 0.9);
  ...
  for (var i = 0; i < this.nTL; i++) {
    ...
    var colArr = middleIsRed
      ? [colGreen, colRed, colGreen]
