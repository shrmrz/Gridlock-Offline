Gridlock-Offline Traffic Simulation

Gridlock-Offline is an advanced traffic simulation application that has been designed to run offline. This application provides users with the ability to simulate their own custom map and select from a range of pre-built maps, all with a new and improved GUI. Our development team has incorporated the latest technologies into the application, expanding beyond the predominantly used JavaScript, HTML, and CSS technologies. During development, we faced a number of challenges, including connecting roads while accommodating various coding styles. Moving forward, we plan to implement a drag-and-drop feature for the maps.
Our project was motivated by the need for more maps that simulate real-life applications. One of our custom maps features an intersection that is known for its heavy traffic, including on-ramp and off-ramp lanes between the busy interstates 78 and 15 in San Diego.
Installation Instructions:
To get started with Gridlock-Offline, please follow these steps:

1. Clone the repository using this command: git clone https://github.com/GridLock-Simulator/Gridlock-Offline.git
2. Navigate to the cloned repository and open the index.html file.
   Features:
   Gridlock-Offline provides a range of features to control traffic flow, including:
   • Adding roadblocks
   • Setting speed limits
   • Installing traffic lights
   Our application is user-friendly and offers an interactive simulation experience. Gridlock-Offline is perfect for anyone interested in traffic simulation, urban planning, or transportation engineering.

About Gridlock-Offline
Gridlock-Offline is an advanced traffic simulation application that has been designed to run offline. This project was created to provide users with a unique and interactive experience in traffic simulation. With the ability to simulate custom maps and pre-built maps with a range of traffic features, Gridlock-Offline is perfect for anyone interested in urban planning, transportation engineering, or traffic simulation.
Our development team has incorporated the latest technologies into the application, expanding beyond the predominantly used JavaScript, HTML, and CSS technologies. During development, we faced a number of challenges, including connecting roads while accommodating various coding styles. Moving forward, we plan to implement a drag-and-drop feature for the maps to improve the user experience.
Gridlock-Offline is an open-source project and we welcome contributions from the community. If you're interested in contributing, please refer to our contribution guidelines. If you encounter any bugs or issues while using the application, please create a new issue on our GitHub repository so that we can address it promptly.
We hope that you enjoy using Gridlock-Offline and find it to be a valuable tool for your traffic simulation needs. Thank you for your support!

Gridlock-Offline
Gridlock-Offline is an advanced traffic simulation application that has been designed to run offline. This application provides users with the ability to simulate their own custom map and select from a range of pre-built maps, all with a new and improved GUI. Our development team has incorporated the latest technologies into the application, expanding beyond the predominantly used JavaScript, HTML, and CSS technologies. During development, we faced numerous challenges, including connecting roads while accommodating various coding styles. Moving forward, we plan to implement a drag-and-drop feature for the maps.
Our project was motivated by the need for more maps that simulate real-life applications. One of our custom maps features an intersection that is known for its heavy traffic, including on-ramp and off-ramp lanes between the busy interstates 78 and 15 in San Diego.
Installation Instructions:
To get started with Gridlock-Offline, please follow these steps:
Clone the repository using this command:
git clone https://github.com/GridLock-Simulator/Gridlock-Offline.git
Navigate to the cloned repository and open the index.html file.
Features:
Gridlock-Offline provides a range of features to control traffic flow, including:
Adding road blocks
Setting speed limits
...
Source Code can be found at Gridlock-Offline
Legacy App for the interactive JavaScript simulation can be found at traffic-simulation.de
...
o in case of obstacles or traffic lights, real or virtual vehicle objects are added to the road at dropping time
o in case of speed limits, no new objects are generated but the vehicle's models are changed.
o in all cases, the visual appearance changes at dropping time
...
• The implementation of the actual models is given in models.js. An extension of the Intelligent-Driver Model ("ACC model") is used as acceleration model, and MOBIL as the lane-changing model. MOBIL has been modified by making its bSafe parameter depending on the speed. Making lane changes more aggressive in congested situations. For further information, see the scientific references below
Graphics
...
More detailed description for road.js
...
• Global or local influence factors on the driving behavior such as overall iter-driver variation, minimum time interval between active and/or passive lane changes, and lane-changing bans. Notice: Speed limits are controlled externally by the TrafficObjects
...
• Function pointers traj containing functions of the geo-referenced x(u) and y(u) coordinates as a function of the arclength u. Notice: This is used purely for graphical reasons.
The most important road functions/methods
• The constructor setting the above attributes and populating the road with a given density and vehicle composition. For a detailed micro initialization, there is the method initializeMicro. For only initializing/resetting the traffic without re-constructing the road or affecting the obstacles, there is the method initRegularVehicles
• updateTruckFrac(frac) Change in situ the percentage of trucks by swapping cars for trucks and vice versa
• updateDensity(density) Change in situ the density by dropping vehicles 'out of thin air' into the largest gaps or randomly removing regular vehicles (the composition is controlled by the global variable fracTruck set by the user
• Add/subtract one lane
• Various searching methods:
o getNearestUof(otherRoad, u): get the longitudinal coordinate of otherRoad that is nearest to the coordinate u on the calling road
o findNearestVehTo(x,y) find on this road the nearest vehicle to a physical (georeferenced) position (x,y)
o findNearestDistanceTo(x,y) Map matching of a geolocated point (x,y) to the calling road. Returned is distance (|v|), u coordinate and v [lanes]
o methods for finding the next leader/follower index or vehicle object for a given longitudinal coordinate u on a given or arbitrary lane Notice: The vehicles are always ordered according to decreasing u, regardless of the lane
• Methods influencing the local driving behavior such as setCFModelsInRange (speed limits), setLCModelsInRange (overtaking bans or anticipation for entering an off-ramp), or setLCMandatory (before lane closings and on onramps)
...
• updateSpeedPositions() Updates speeds by the Euler method and positions by the ballistic method (see section Numerical Integration).
• changeLanes() tests and executes lane changes first to the right, then to the left. Because of the waiting times after each active or passive lane change (state variables vehicle.dt_afterLC, vehicle.dt_lastPassiveLC and road.waitTime), changes to the right are prioritized and side effects are avoided
• mergeDiverge(otherRoad,...) change to another network link from the calling element to otherRoad if this other element has a parallel section with the calling road (onramp or offramp). Parameters include the offset of the arc-length (u) coordinate new-old road, the region uBegin and uEnd of the ramp, whether it is a merge, whether it is to the right.
Notice: Since the vehicle transfer is always calling from one road to the other road, it is always a diverge. However, merges are always at the end of the calling road and have a standing virtual obstacle at its end. Moreover, merging affects all vehicles while diverging takes place only if the corresponding vehicle route have the new road as next element or (if ignoreRoute is true) for the vehicles on the adjacent lane.
• connect(..) and determineConflicts: These methods will be considered in their own section
...
o The buffer is incremented by Qin\*dt with some noise and decremented by 1 if a new vehicle enters or the maximum buffer size (at the present 2) is exceeded.
o The type is determined based on the present global fractruck variable, and the inter-driver variation is set when constructing the vehicle.
o The vehicle is set at the lane with the largest gap unless it is a truck. Then it is set preferably to the right
...
• updateSpeedlimits(trafficObjects) If the user dragged a speed limit to a new position, lifted one, or changed its value. Notice: Since dragging is cumbersome on touch devices, the scenarios roadworks where limits are crucial has also a slider for the speedlimit which is changed globally at updateModelsOfAllVehicles
...
o update the truck fraction
o update the models
o update the density (only for the ringroad scenario)
...
This is realized by the method road.connect(target, uSource, uTarget, offsetLane, conflicts, options). When connecting two network elements end-to-end (for example to model lane closing or opening or other changes of the road properties or right turns where the only thing to watch are the vehicles on the target road but no crossing streams), conflicts=[]. Otherwise, the conflicts are analyzed by road.determineConflicts(..)
Connecting two roads end to end
• If this is used to connect two roads with the same number of lanes but possibly different other properties, you just call sourceRoad.connect(targetRoad, source.roadLen,0,0,[]);
...
• You could also simultaneously subtract a lane on the right and add one on the left by setting equal lane numbers for the source and target and offsetLane=+1. In all cases, vehicles change lanes to continue the through lanes in advance.
...
• Crossing a main road straight-on: traffic on the two mainroad directions (=two separate directed link of the road type). The left-turners from the opposite road have to care for themselves, so there is no conflict potential
• Crossing an equal-rank intersection (right priority) straight-on: traffic on the main road direction coming from the right, except right-turners because they are eventually on the target road. Notice: For that to work, the vehicles change to the new logical link ahead of the actual passing time of the physical boundaries.
• Left turn from a priority road: Straight-ahead OD of the opposite direction of the priority road (the left turning traffic from the opposite main road does not conflict, while the right turning traffic has the same target as the subject and is therefore taken care of as a target-road vehicle).
