...
function TrafficObjects(canvas, nTL, nLimit, xRelDepot, yRelDepot, nRow, nCol) {
  ...
  for (var i = 0; i < this.n; i++) {
    ...
    var img = isTL
      ? this.imgTLred
      : isSpeedl
      ? this.imgSpeedlRepo[initSpeedInd[iSpeed]]
      : this.imgObstRepo[iObst];
    if (debug) {
    ...
    if (
      this.trafficObj[i].type == "speedLimit" &&
  ...
  if (debug) {
    ...
    for (var i = 0; i < this.trafficObj.length; i++) {
      console.log(
        "TrafficObjects cstr: i=",
        i,
        " value=",
        this.trafficObj[i].value,
        " type=",
        this.trafficObj[i].type,
        " id=",
        this.trafficObj[i].id,
        " imgfile=",
        this.trafficObj[i].image.src,
        " isActive=",
        this.trafficObj[i].isActive
      );
    }
  }
...
TrafficObjects.prototype.calcDepotPositions = function (canvas) {
  ...
  for (var i = 0; i < this.n; i++) {
    ...
    this.trafficObj[i].xPixDepot =
      xPixDepotCenter + (this.wPix + gapPix) * (icol - 0.5 * (this.nCol - 1));
    this.trafficObj[i].yPixDepot =
      yPixDepotCenter + (this.hPix + gapPix) * (irow - 0.5 * (this.nRow - 1));
...
TrafficObjects.prototype.draw = function () {
  void (debug && console.log("in TrafficObjects.draw"));
  ...
  for (var i = 0; i < this.trafficObj.length; i++) {
    ...
    if (
      ...
      if (this.active_drawTopSign) {
        // draw active sign above the road
      ...
      if (this.active_drawBotSign) {
        // draw active sign below the road
        v *= -1;
        xPix = xCenterPix + scale * v * sphi; // + left if cphi>0
        yPix = yCenterPix + scale * v * cphi; // -*-=+
        ctx.setTransform(1, 0, 0, 1, xPix, yPix);
      ...
      if (debug) {
        console.log(
          "TrafficObjects.draw active obj: i=",
          i,
          " type=",
          obj.type,
          " obj.u=",
    ...
    if (!obj.isActive) {
      ...
      if (debug) {
        ...
        console.log(
          "time=",
          time,
          " in TrafficObjects.draw: passive objects: id=",
          obj.id,
          " value=",
          obj.value,
          //" fname=",obj.image.src,
...
TrafficObjects.prototype.selectByUser = function (
  xPixUser,
  yPixUser,
  distCritPix
) {
  ...
  for (var i = 0; i < this.trafficObj.length; i++) {
    var dist2 =
      Math.pow(xPixUser - this.trafficObj[i].xPix, 2) +
  ...
  if (debug) {
    if (success) {
      console.log(
        ...
        " isActive=",
        trafficObjreturn.isActive,
        " xPixUser=",
        formd0(xPixUser),
        " yPixUser=",
        formd0(yPixUser),
        " xPix=",
        formd0(this.trafficObj[i_opt].xPix),
        " yPix=",
        formd0(this.trafficObj[i_opt].yPix),
        "end"
      );
    } else {
      console.log(
        "  no success",
        " nearest object has type",
        this.trafficObj[i_opt].type,
        " xPixUser=",
        formd0(xPixUser),
        " yPixUser=",
        formd0(yPixUser),
...
TrafficObjects.prototype.activate = function (obj, road, u) {
  ...
  if (!(typeof u === "undefined")) {
    // external setting; must take care of all
    obj.u = u;
...
TrafficObjects.prototype.deactivate = function (obj) {
  ...
  if (obj.isActive) {
    if (obj.type === "trafficLight") {
      void (debug && console.log("TrafficObjects.deactivate"));
...
TrafficObjects.prototype.pickObject = function (
  xPixUser,
  yPixUser,
  distCritPix
) {
...
TrafficObjects.prototype.dropObject = function (
  obj,
  network,
  xUser,
  yUser,
  ...
  void (
    ...
    console.log(
      "itime=",
      itime,
      " in TrafficObjects.dropObject: obj.id=",
      obj.id,
      " obj.xPix=",
      obj.xPix,
      " network[0].roadID=",
  ...
  void (
    debug &&
    console.log(
      "inside dropObject begin: this.trafficObj[0]=",
      this.trafficObj[0]
    )
  );
  ...
  for (var iroad = 0; iroad < network.length; iroad++) {
    ...
    void (
      debug &&
      console.log(
        "  TrafficObjects.dropObject: iroad=",
        iroad,
        " dropInfoNearest=",
        dropInfoNearest,
        " distMin=",
    ...
    void (
      debug && console.log("  end iroad loop cmds: iroadNearest=", iroadNearest)
    );
  }
  ...
  if (true) {
    void (
      debug &&
      console.log(
        "  end TrafficObjects.dropObject: success=",
        success,
        " nearest roadID=",
        road.roadID,
        " road.roadLen=",
  ...
  void (
    debug &&
    console.log(
      "inside dropObject end: this.trafficObj[0]=",
      this.trafficObj[0]
    )
  );
}; // dropObject
...
TrafficObjects.prototype.selectSignOrTL = function (xPixUser, yPixUser) {
  if (debug) {
    console.log(
      "in TrafficObjects.selectSignOrTL:",
      " xPixUser=",
      xPixUser,
      " yPixUser=",
      yPixUser
    ...
    if (
      ...
      if (debug) {
        console.log(
          ...
          formd0(obj.yPix),
          " obj.xPixSign1=",
          formd0(obj.xPixSign1),
          " obj.xPixSign2=",
          formd0(obj.xPixSign2),
          " distPix=",
          formd0(distPix),
          " distPix1=",
          formd0(distPix1),
...
TrafficObjects.prototype.setTrafficLight = function (obj, value) {
  void (
    debug &&
    console.log(
      "in TrafficObjects.setTrafficLight, obj=",
      obj,
      " value=",
      value
    )
  ...
  if (!(obj.type === "trafficLight")) {
    void (
      debug &&
      console.log(
        "TrafficObjects.setTrafficLight: error:",
  ...
  if (obj.isActive) {
    // then, obj has a road reference
    obj.road.changeTrafficLight(obj.id, obj.value);
  }
  /*
    if (false) {
      if (obj.isActive) {
        void (debug && console.log(
          "setTrafficLight:  id=",
          obj.id,
          " road ID=",
          obj.road.roadID
...
TrafficObjects.prototype.changeTrafficLightByUser = function (
  xPixUser,
  yPixUser
) {
  void (
  ...
  void (debug && console.log("  selectSignOrTL results=", results));
  ...
  if (results[0]) {
    ...
    if (obj.type === "trafficLight") {
      ...
      if (false) {
        void (debug && console.log(
          "end TrafficObjects.changeTrafficLightByUser: ",
        ...
        if (obj.isActive) {
          void (debug && console.log("  on road ID ", obj.road.roadID));
          obj.road.writeTrafficLights();
        }
      }
    */
...
TrafficObjects.prototype.setSpeedlimit = function (obj, value) {
  if (!(obj.type === "speedLimit")) {
    void (
      debug &&
      console.log(
        "TrafficObjects.setSpeedlimit: error:",
  ...
  if (value < 5) {
    index = 1;
  } // no 5 km/h image at present
  if (value >= 125) {
    index = 0;
  } // free sign/no speed limit
...
TrafficObjects.prototype.zoomBack = function () {
  ...
  for (var i = 0; i < this.trafficObj.length; i++) {
    ...
    if (!obj.isActive && !obj.inDepot && !obj.isDragged && !obj.isPicked) {
      ...
      if (debug) {
        console.log(
          "TrafficObjects.zoomBack: i=",
          i,
          " obj.xPix=",
          obj.xPix,
          " obj.xPix=",
          obj.xPix,
          " this.trafficObj[i].xPix=",
          this.trafficObj[i].xPix
        );
      }
    }
...
TrafficObjects.prototype.drag = function (xPixUser, yPixUser) {
  void (debug && console.log("in TrafficObjects.drag"));
};
...
TrafficObjects.prototype.writeObjects = function (onlyTL) {
  ...
  void (
    ...
    console.log(
      "itime=",
      itime,
      " in TrafficObjects.writeObjects:",
      " justTL=",
      justTL,
      ":"
    )
