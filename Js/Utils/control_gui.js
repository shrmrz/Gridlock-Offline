...
var nLevels = 6;
function showInfo() {
  var scenarioFile = "/Docs/Info/info_" + scenarioString + ".html";
  void (
    debug && console.log("showInfo (control_gui): scenarioFile=", scenarioFile)
  );
...
function addOneLane() {
  ...
  void (debug && console.log("addOneLane: mainroad.nLanes=", mainroad.nLanes));
  if (mainroad.nLanes === nLanesMax) {
    document.getElementById("lanePlusDiv").classList.add("hide");
  }
  if (mainroad.nLanes > nLanesMin) {
    document.getElementById("laneMinusDiv").classList.remove("hide");
  }
}
...
function subtractOneLane() {
  ...
  void (
    debug && console.log("subtractOneLane: mainroad.nLanes=", mainroad.nLanes)
  );

  if (mainroad.nLanes === nLanesMin) {
    document.getElementById("laneMinusDiv").classList.add("hide");
  }
  if (mainroad.nLanes < nLanesMax) {
    void (debug && console.log("in setting lanePlus visible!!!"));
    document.getElementById("lanePlusDiv").classList.remove("hide");
  }
}
