...
// id's defined mainly in vehicle.js
...
function handleTouchStart(evt) {
  ...
  if (true) {
    ctx.beginPath();
    ctx.arc(scale * xUser, -scale * yUser, 4, 0, 2 * Math.PI, false); // a circle at the start
...
function showLogicalCoords(xPixUser, yPixUser) {
  ...
  var coordsStr_ylb = 0.9 * canvas.height;
  ...
  ctx.fillRect(
    coordsStr_xlb,
    coordsStr_ylb - coordsStr_height,
    coordsStr_width,
    coordsStr_height
  );
...
function pickRoadOrObject(xUser, yUser) {
  ...
  */
  ...
  if (!(typeof trafficObjs === "undefined")) {
    // just check for scenarios w/o
    var distCrit_m = 20; //[m] !! make it rather larger
    var pickResults = trafficObjs.pickObject(
      xPixUser,
      yPixUser,
      distCrit_m * scale
    );
    ...
    if (pickResults[0]) {
      ...
      if (debug) {
        console.log(
          ...
          trafficObject.type,
          " isActive=",
          trafficObject.isActive,
          " inDepot=",
          trafficObject.inDepot,
          " end"
        );
      }
      return;
...
function doDragging(xUser, yUser, xUserDown, yUserDown) {
  if (mousedown || touchdown) {
    ...
    if (distDrag > distDragCrit) {
      // !! do no dragging actions if only click
      if (trafficObjPicked) {
        // dragged an object
...
function finishDistortOrDropObject(xUser, yUser) {
  if (debug) {
    console.log(
      "itime=",
      itime,
      " in finishDistortOrDropObject (canvas_gui):",
      " trafficObjPicked=",
      trafficObjPicked,
      " roadPicked=",
      roadPicked,
...
function handleClick(event) {
  ...
  if (debug) {
    ...
    console.log(" isDragged=", isDragged, " speedlBoxActive=", speedlBoxActive);
  }
...
function handleClick_golfCourse(event) {
  var distMax = 0.1 * refSizePhys; // 0.05*smaller side in meters
  ...
  if (debug) {
    console.log(
      "\n\nitime=",
      itime,
      " in handleClick_golfCourse: xPixUser=",
      formd0(xPixUser),
      " yPixUser=",
      formd0(yPixUser),
      " xUser=",
...
function activateSpeedlBox(xPixUser, yPixUser) {
  ...
  var relWidth = 0.1; // rel size and position of the graphical select box
  var relHeight = 0.025 * speedlBoxAttr.limits.length; // rel to the smaller dim
  var relDistx = 0.1; // center-center
  var relDisty = 0.0;
...
function changeSpeedl(xPixUser, yPixUser) {
  ...
  if (speedlBoxActive) {
    if (
      xPixUser > speedlBoxAttr.xPixLeft &&
