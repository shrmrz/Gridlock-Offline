...
road.prototype.subtractOneLane = function () {
  for (var i = this.veh.length - 1; i >= 0; i--) {
    void (
      debug && console.log("road.subtractOneLane: old nLanes=", this.nLanes)
    );
...
road.prototype.setOfframpInfo = function (
  offrampIDs,
  offrampLastExits,
  offrampToRight
) {
...
road.prototype.connect = function (
  ...
  var targetHasPiority = typeof targetPrio === "undefined" ? false : targetPrio;
  ...
  if (false) {
    console.log(
      "\n\nbegin road.connect: t=",
      time.toFixed(2),
      " this.roadLen=",
      this.roadLen.toFixed(1),
      " targetRoad.roadID=",
      targetRoad.roadID,
      " uSource=",
      uSource.toFixed(1),
...
*/
      ...
      if (connecting) {
        ...
        if (createVirtStandingVehs) {
          for (var il = 0; il < this.nLanes; il++) {
            var laneContinues =
              il + offsetLane >= 0 && il + offsetLane < targetRoad.nLanes;
            if (!laneContinues) {
              nVirtStandingVehs++;
              var virtualStandingVeh = // vehicle(len,width,u,lane,speed,type)
                new vehicle(0.01, laneWidth, this.roadLen, il, 0, "obstacle");
        ...
        var uGo =
          uSource -
          (distBeforeEnd +
            Math.max(1.2 * s0, 0.2 * (duDecision - distBeforeEnd)));
        ...
        if (!laneContinues) {
          ...
          if (vehConnect.dt_afterLC <= 0.5 * dt) {
            ...
            var laneContinuesOld =
              laneOld + offsetLane >= 0 &&
        ...
        if (laneContinues) {
          ...
          var followerInfo = targetRoad.findFollowerAtLane(
            uTarget,
            lane + offsetLane
          );
          ...
          var uFollow = followerExists ? targetRoad.veh[iFollow].u : -1000000;
          var speedFollow = followerExists ? targetRoad.veh[iFollow].speed : 0;
          var sFollow = followerExists
            ? uTarget - (uSource - u) - vehConnect.len - uFollow
            : 1000000;
          var leaderInfo = targetRoad.findLeaderAtLane(
            uTarget,
            lane + offsetLane
          );
          ...
          var uLead = leaderExists ? targetRoad.veh[iLead].u : +1000000;
          ...
          var sLead = leaderExists
            ? uLead - uTarget + (uSource - u) - lenLead
            : 100000;
          ...
          if (C2 || C3) {
            ...
            if (followerExists) {
              // true follower at uTarget at present

              // true if anticipated follower is the same
              var anticipatedFollowerExists =
                uFollow + speedFollow * tc < uTarget;

              // see comments at "update_iLead" no follower if iLag<=iFollow
              var iLag = targetRoad.veh[iFollow].iLag;

              if (false) {
                // only if look at determining new antic follower
                //if(this.connectLog){
                console.log(
                  ...
                  "uTarget=" + uTarget.toFixed(1),
                  " anticipatedFollowerExists=" + anticipatedFollowerExists,
                  " iLag=" + iLag,
                  " iFollow=" + iFollow,
                  ""
                );
            ...
            if (anticipatedFollowerExists) {
              ...
              if (false) {
                console.log(
                  "before crash: veh id=" + vehConnect.id,
                  " sFollow=" + sFollow,
                  " uFollow=" + uFollow,
                  " targetRoad.veh[iFollow]=" + targetRoad.veh[iFollow]
                );
              }
              ...
              if (leaderExists) {
                ...
                sLead =
                  uLead - uTarget + (uSource - u) - lenLead + speedLead * tc; // !! antic
              }
              if (false) {
                //if(this.connectLog){
                console.log(
          ...
          if (this.connectLog) {
            console.log(
              "  Prep (ii): Potential free acceleration if",
          ...
          if (leaderExists) {
            ...
            accGo = Math.min(
              accDefault,
              vehConnect.longModel.calcAccDet(sLead, speed, speedLead, accLead)
            ); // !! influence
            if (this.connectLog) {
              console.log(
                "  Prep (iiia): leaderExists: leader id=" +
                  targetRoad.veh[iLead].id,
                " s=" + s,
                " accGo=" + accGo,
                " = min(free acc, target leader acc)"
              );
            }
            ...
            if (potentialConflictsExist) {
              // only reference to conflicts here
              ...
              for (var ic = 0; ic < conflicts.length; ic++) {
                ...
                if (this.connectLog) {
                  console.log(
                    "  Preparation (iiib): ic=" + ic,
                    "conflicts[ic]=" + conflicts[ic]
                  );
                }
              }
          ...
          if (followerExists) {
            if (C2 || C3) {
              // decision zone, zone C1 was braking zone
          ...
          if (false) {
            //if(this.connectLog){
            console.log(
              "  Prepare Action A4 assuming no conflicts" +
          ...
          // updated
            ...
            if (this.connectLog) {
              console.log(
                "  Action A2: potential conflicts possible" +
                  //" C2="+C2," C3="+C3,
                  //" conflictsExist="+conflictsExist,
          ...
          if (!(conflictsAllowPassing && targetCanBeEntered)) {
            ...
            if (this.connectLog) {
              console.log(
                "  Action 3: conflicts or gridlock danger:" +
          ...
          // for jam propagation
            ...
            if (this.connectLog) {
              console.log(
                "  Action 4: ready to go" + " veh id=" + vehConnect.id,
                " route=" + vehConnect.route,
                " targetID=" + targetID,
        ...
        if (u > uSource - distBeforeEnd && this.roadID != targetRoad.roadID) {
          ...
          if (this.connectLog) {
            ...
            console.log(
              "  Action 5: All previous actions drove the vehicle" +
...
road.prototype.mergeDiverge = function (
    ...
    for (var i = 0; i < originVehicles.length && !success; i++) {
      ...
      if (
        ...
        if (success && log) {
          ...
          console.log(
            "  acc=" + acc + " accNew=" + accNew + " accLagNew=" + accLagNew
          );
          console.log(
            "  duLeader=" +
              duLeader +
  ...
  if (success) {
    ...
    if (false) {
      //if(true){
      console.log(
        "Actual merging: merging origin vehicle " +
          iOrig +
          " of type " +
          this.veh[iOrig].type +
      ...
      console.log(
        " this.veh[iOrig].divergeAhead)=" + this.veh[iOrig].divergeAhead
      );
    }
  ...
  for (var i = 0; i < this.veh.length; i++) {
    if (this.veh[i].isRegularVeh()) {
      this.veh[i].longModel.copy(
        this.veh[i].type === "car" ? longModelCar : longModelTruck
      );
      this.veh[i].LCModel.copy(
        this.veh[i].type === "car" ? LCModelCar : LCModelTruck
      );
      ...
      if (this.veh[i].isRegularVeh()) {
        ...
        if (nextOfframpNearby) {
          ...
          if (tacticalLC) {
            ...
            thisVeh.longModel.alpha_v0 = Math.max(
              0.1,
              (0.5 * duRemaining) / this.duTactical
            );

            thisVeh.LCModel = toRight
            ...
            if (false) {
              console.log(
                "road.updateModelsOfAllVehicles: apply tacticalLC to Veh" +
                  ...
                  toRight +
                  " u=" +
                  parseFloat(thisVeh.u).toFixed(1) +
                  " uLastExit=" +
                  parseFloat(uLastExit).toFixed(1) +
                  " bBiasRight=" +
                  thisVeh.LCModel.bBiasRight
              );
            }
          }
        } // nextOfframpNearby
...
road.prototype.draw = function (
  ...
  if (!movingObserver && this.drawAlternativeTrajectories) {
    for (var iTraj = 0; iTraj < this.trajAlt.length; iTraj++) {
      ...
      var wSegmPix =
        scale * (laneMax - laneMin + 1) * this.laneWidth + boundaryStripWidth;
...
road.prototype.drawVehicles = function (
  ...
  if (false) {
    console.log(
      "road.drawVehicles: traj init:" + " this.usedTraj_x=" + this.usedTraj_x
    );
  }
  ...
  for (var i = 0; i < this.veh.length; i++) {
    ...
    if (filterPassed) {
      ...
      this.drawVehicle(
        ...
        speedmin,
        speedmax,
        [this.usedTraj_x, this.usedTraj_y],
        xOffset,
        yOffset,
        upright
      );
...
road.prototype.drawVehicle = function (
  ...
  if (type != "obstacle" && speedmax > 1e-10) {
    ...
    var effLenPix = type === "car" ? 0.95 * vehLenPix : 0.9 * vehLenPix;
    var effWPix = type === "car" ? 0.55 * vehWidthPix : 0.7 * vehWidthPix;
    var speed = this.veh[i].speed;
...
road.prototype.dropObject = function (trafficObj) {
  ...
  // NOTICE: traffic light has its sorting/pushing/splicing methods
    ...
    void (debug && console.log("  end road.dropObject: added traffic light"));
  } else {
...
road.prototype.removeObstacle = function (id) {
  ...
  void (debug && console.log("in road.removeObstacle: id=" + id));
  var success = false;
  var iDel = -1;
